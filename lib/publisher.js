...
var BUFFER_DRAIN_INTERVAL = 1000;
...
Publisher.prototype.publish = function (message, options, routingKey, callback) {
  ...
  if (!this._channel || this._buffer) {
    this._queue.push({message: message, options: options, routingKey: routingKey, callback: callback});

    if (!this._drainTimeout && !this._buffer && !this._draining) {
      // trigger polling for queue drain
      this._drainTimeout = setTimeout(this.drainQueue.bind(this), BUFFER_DRAIN_INTERVAL);
    }
...
Publisher.prototype.drainQueue = function () {
  ...
  if (!this._channel) {
    // poll until the channel is established
    this._draining = false;
    this._drainTimeout = setTimeout(this.drainQueue.bind(this), BUFFER_DRAIN_INTERVAL);
    return;
  }
  ...
  this.publish(item.message, item.options, item.routingKey);

  process.nextTick(this.drainQueue.bind(this));
};