...
Publisher.prototype.getUrl = function () {
  return 'amqp' + (this.options.ssl.enable ? 's' : '') + '://' + this.options.user + ':' + this.options.password + '@' + this.options.host + ':' + this.options.port + '/' + encodeURIComponent(this.options.vhost) + (this.options.ssl.enable ? '?' + queryString.stringify(_.omit(this.options.ssl, 'enable')) : '');
};
...
Publisher.prototype.handleError = function (err, cb) {
  ...
  setTimeout(this.connect.bind(this, cb), this.connectWait).unref();
};
 ...
 * @param {function} [callback] will be called after message has been published
 */
